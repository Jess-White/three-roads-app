...
Win test: Take in a string input "2 3 11 + 5 - *" and 
return a single integer that is the postfix sum of all numbers and operands in the input, 18
...
describe('finalTest', () => {
  const calculate = (expression) => {
    ...
    expression.split(" ").forEach(element => {
      // if element is +, -, *, then do whatever
      if (operands.includes(element)) {
        const right = ourArray.pop();
        const left = ourArray.pop();
        // do addition if the operand is "+"
        if (element === "+") {
          ourArray.push(right + left)
        } else if (element === "-") {
          ourArray.push(left - right)
        } else if (element === "*") {
          ourArray.push(left * right)
        }
      } else {
        ourArray.push(parseInt(element))
      }
      // else we know it's a number so we can convert to an integer and put it in the array
      // parseInt 
    });
  ...
  it('should take in a string input "1 2 +" and return a single integer, 3', () => {
    const finalTestSum = calculate("1 2 +");
    expect(finalTestSum).toEqual(3);
  });
  it('should take in a string input "1 2 -" and return a single integer, -1', () => {
    const finalTestSum = calculate("1 2 -");
    expect(finalTestSum).toEqual(-1);
  });
  it('should take in a string input "1 2 *" and return a single integer, 2', () => {
    const finalTestSum = calculate("1 2 *");
    expect(finalTestSum).toEqual(2);
  });
  it('should take in a string input "2 3 11 + 5 - *" and return a single integer, 18', () => {
    const finalTestSum = calculate("2 3 11 + 5 - *");
    expect(finalTestSum).toEqual(18);
  });
});
